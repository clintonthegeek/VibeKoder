...
VibeKoder needs its own custom session file format. This will simplify parsing, and will allow for syntax highlighting. I'd like to create a syntax definition file adhering to the KDE Frameworks Syntax Highlighting format. That way, I can use Kate and KatePart-based widgets to get all the blocks cleanly highlighted.

Here is the XML schema for Kate's syntax highlighting, in full:
```xml
...
I'd like our VibeKode session format to be very, very close to Markdown's syntax, since Markdown is what LLMs natively respond with anyway. What is especially useful is the code fencing.

The main change will be the creation of a top-level delimiter line, or tag, for cleanly, unambiguously dividing prompt slices. Our line should contain a string of prompt-slice specific metadata including role (System, User, or Assistant) and date and time. Our current header does that, easily parsable. If the user hand-writes the line, they needn't add the date and time, just the role, and the date and time should be added automatically by the parser when the Send button is pressed. The date and time in the delimiter line, with this metadata, will act as the UID, or key, for each prompt-slice in the session, allowing navigation, etc.

I'm thinking that the delimiter line should look like this:

```
=={ Assistant | 2025-06-02 21:16:38 }==
```

Everything *in* those slices will be markdown, as usual. Right now our document itself is markdown, using top-level `#` Markdown headers to divide slices. These are very ambiguous and lead to a great deal of complications, like nesting, etc.

I'd like there to be a header for various metadata. At the top of the file, have a YAML-like metadata block enclosed in triple dashes:
  ```yaml
  ---
  project: MyProject
  session: 006
  ...
  ---
  ```
I don't know what we'll put in the block yet, so let's handle this as generically as possible. To handle a block of data with arbitrary keys in Qt, you can use a `QVariantMap`, which acts like a flexible dictionary mapping string keys to values of various types. As you parse each key-value pair from your input block, dynamically insert them into the `QVariantMap` by using the parsed key as the map key and wrapping the value in a `QVariant`. This way, you don’t need to predefine the keys in your code; instead, the map grows to reflect exactly the structure of the data you read. Later, you can access or manipulate these values by their string keys, making `QVariantMap` an ideal container for storing and working with ad-hoc, dynamically structured data in Qt.

I expect slices to contain arbitrary Markdown, including code blocks, lists, tables, etc., so the original markdown syntax parser should remain as robust as it already is.

We dont' need nested slices, just a flat list. Nesting will be handled by the soon-coming functionality of branching into separate files. 

I don't need backward compatibility or conversions from the old format; the delimiter line will be the *only* way to separate slices. The parser should automatically add timestamps to the final delimiter line, assumed to be written by the user, if missing. Data in the delimiter line should be one line. 
